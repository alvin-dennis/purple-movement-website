...
export function RadialFlow({ topics, badgeName, centralDotColor = "#FFFFFF" }: RadialFlowProps) {
  ...
  const getTopicLabelClasses = useCallback(
    (topic: Topic) =>
      `absolute transform -translate-x-1/2 -translate-y-1/2 bg-card/80 text-foreground rounded-md 
     backdrop-blur-md border transition-all duration-300 px-3 py-2 text-[10px] sm:text-xs font-medium
     ${
       topic.highlighted
         ? "border-primary/50 shadow-[0_0_15px_rgba(120,81,169,0.2)] dark:border-primary/50 dark:shadow-glow"
         : "border-border/50"
     }
  ...
  return (
    <div ref={containerRef} className="w-full h-full  relative overflow-hidden min-h-75">
        ...
        >
          ...
          {topics.map((topic) => (
            <path
              key={`path-${topic.id}`}
              d={getPathData(topic)}
              stroke={topic.highlighted ? topic.color : "currentColor"}
              strokeWidth={topic.highlighted ? "1.5" : "1"}
              className={topic.highlighted ? "" : "text-border/40 dark:text-border/20"}
              strokeOpacity={topic.highlighted ? 0.6 : 0.3}
              fill="none"
            />
          ))}
